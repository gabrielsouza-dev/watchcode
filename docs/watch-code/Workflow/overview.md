...
| E0-T6 | Erro no log ao esconder as Extensões | Esconder o container `workbench.view.extensions` deixou a viewlet de Extensões registrando views num container que já não existe: `ExtensionsViewletViewsContribution` faz `.get(VIEWLET_ID)!` — que devolve `undefined` — e chama `registerViews(views, undefined)`; o `ViewDescriptorService` estoura em `viewContainer.id` (`viewDescriptorService.js:174`) e o erro é gravado no log em **toda** abertura | E0-T4 | pendente — encontrado em 16/09/2026 numa exploração do produto aberto e medido depois: a falha está nos logs de **todos** os perfis de execução do arnês. Não derruba o app (o `safeCreateContribution` engole a exceção), mas é erro de verdade no log de subida. Candidatos: guardar a contribuição do core quando o container não existe (3 linhas) ou esconder as Extensões por outro caminho que não o `deregisterViewContainer` |
...
| E2-T8 | Só o que mudou | Esconder no Explorer os arquivos que o agente não tocou, com o atalho **F7**; a view própria é a opção recomendada sobre o gancho no core do Explorer | E2-T7 | feito (13 testes novos no módulo (`watchCode`, 53 → 66); manual T-0014 aprovado com **23 conferências**, e o arnês inteiro com os **catorze** cenários verdes. A forma é a **view própria** — a árvore `Changed Only`, dentro do container do Explorer e recolhida, com o **F7** abrindo e focando e recolhendo quando já está em uso; o Explorer de verdade continua mostrando tudo, e **nenhum arquivo de `contrib/files` foi tocado**. As pastas da árvore são derivadas dos caminhos dos eventos, nunca do disco, e o nó do arquivo carrega a alteração mais recente, que é a que o salto abre. No caminho, o trecho de abertura da lista virou módulo compartilhado (`ChangeOpener`), usado pelas duas views em vez de duplicado) |
| E2-T9 | Linha da timeline sem texto cortado | A segunda faixa da linha aparece inteira: a lista para de escrever a altura do item na entrelinha dele (`setRowLineHeight: false`) | E2-T2 | feito (defeito visual encontrado pelo usuário em 16/09/2026, com o produto aberto num projeto de exemplo, e medido antes e depois por sonda sobre o depurador; cenário **T-0017** no arnês com 5 conferências verdes. A entrelinha da faixa de detalhe era a altura do item — 44 px dentro de uma caixa de 22 px — e os glifos saíam cortados ao meio em **toda** linha da lista. Era invisível para os testes: o texto está inteiro no DOM, e as conferências do T-0005, do T-0012 e do T-0013 leem texto) |
...
**Provado na E2-T7:** a árvore de arquivos passou a contar o que o agente fez, sem depender da linha do
tempo. No app, com as escritas vindas de fora: o arquivo tocado nasce na árvore com **cor e ponto** — e a
conferência é feita com a view da timeline ainda **recolhida**, provando que a decoração vem do serviço e
não da lista; ir até a alteração pelo F5 apaga o ponto e **mantém a cor**; escrita nova num arquivo já
visto faz o ponto **voltar**; a pasta sinaliza enquanto houver pendência embaixo dela e volta ao normal
depois da visita; e o arquivo que a observação nunca viu — escrito antes de o app subir — não tem
decoração nenhuma. As linhas desenhadas foram conferidas contra o ledger, evento por evento, e o rótulo
saiu com o texto do produto: `...\aquecimento.ts • Changed by the agent`.

**O que a E2-T7 deixou medido:** a decoração passa pelo `IDecorationsService`, então **divide o rótulo
com o git** — num arquivo que também está modificado no git, o nosso ponto toma o lugar da letra do git e
a nossa cor vence a dele, com o tooltip dos dois provedores aparecendo junto (preço registrado na SPEC).
Ficou medido também que contribuição de workbench só existe se o módulo for importado em
`workbench.common.main.ts`: o provedor ficou invisível até o import entrar, e foi a primeira execução do
...
deles criando o nó da pasta — e um terceiro, escrito com a árvore **recolhida**, já estava nela na primeira
vez que a view abriu, que é a prova de que a escuta nasce na construção e não no desenho. O clique na linha
e o Enter na linha focada abriram a alteração na linha 2 com a faixa selecionada e gravaram o `viewedAt` no
**próprio evento**, e o ponto sumiu sem o arquivo sair da árvore. O arquivo que o agente apagou continuou na árvore marcado como removido, e abri-lo avisou em vez de
...
**O defeito que a E2-T9 corrigiu:** a lista da timeline era a única do workbench com item
desenhado à mão que **não** desligava a entrelinha de item — as outras 21 já desligavam. Sem
isso a lista escreve a altura do item (44 px) na entrelinha dele (`listView.ts:1073`), e cada
uma das duas faixas de texto do nosso item nascia com 44 px de entrelinha dentro de uma caixa
de 22 px: o texto saía cortado ao meio e a linha invadia a de cima. Passou por **todos** os
testes da E2 porque eles leem o texto das linhas, e o texto estava lá — inteiro, no DOM. Quem
pegou foi o olho do usuário, com o produto aberto; quem passou a pegar é o **T-0017**, que mede
caixa contra conteúdo em vez de ler texto.
